...
# Use gcc -march=native -Q --help=target for march info

if [ ! -d "~/haproxy-2.3" ]; then
  git clone -b master http://git.haproxy.org/git/haproxy-2.3.git/
  pushd haproxy-2.3
  make clean && make TARGET=linux-glibc USE_OPENSSL=1 USE_ZLIB=1 USE_PCRE2=1 \
    CC=/usr/bin/gcc10-gcc CPU_CFLAGS="-march=native -O3" CPU=custom V=1
  ./haproxy -vv
  popd
fi
